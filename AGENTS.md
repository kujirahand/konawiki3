...
- 機能を追加したり、修正したら、テスト(kona3engine/tests/*.test.php)を追加します。テストを実行するには`just test`を実行します。
- PHPの標準コーディングスタイルに準拠します。
...
一般的なMarkdown形式をサポートしています。
拡張表記として、`!!プラグイン(引数)` も使用できます。

### パーサーエンジンの切り替えについて

- デフォルトは、設定ファイルにて。
- ページが `.md` 拡張子の場合、Markdownとして解析されます。
- また、メタデータ`.meta/ページ名.json`で、`mode`でも指定できます。
