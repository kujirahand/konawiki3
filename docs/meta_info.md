...
`{タイトル}.meta.json` ファイルには、ページに関連するメタ情報がJSON形式で保存されます。例えば、タグ情報は以下のように保存されます。
...
メタ情報は、各ページに対応する `.meta.json` ファイルとして保存されます。

```
data/
  ├── PageName.txt          # ページの本文
  ├── PageName.meta.json    # ページのメタ情報
  ├── AnotherPage.md        # Markdownページ
  └── AnotherPage.meta.json # メタ情報
```
